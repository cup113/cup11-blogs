...
> 代码并不是一次性的。许多代码要在各处被重复使用，我们用函数来封装这些代码，增强**可维护性**与**可共享性**。
...
箭头函数（Arrow Function）是 ES6 引入的语法糖，简化了函数定义，尤其适用于回调函数。
...
:::warning
**注意**：箭头函数没有自己的 `this`，它继承自外层作用域，因此不适合用作对象的方法。
:::
...
**函数提升（Hoisting）**：函数声明会被提升到作用域顶部，因此可以在定义前调用。函数表达式不会被提升。

函数声明会被提升，而函数表达式和箭头函数不会。
...
<script>
  ...
  console.log(invalidArea); // 输出: NaN
</script>
```
...
<script>
  ...
  console.log(mainDiv.textContent); // 输出: 主内容
</script>
```
...
:::warning
**大小写敏感**：`getElementById` 和 `getElementsByClassName` 对大小写敏感，需确保与 HTML 中的 ID 或 class 名完全匹配。
:::
...
- `querySelectorAll` 返回 NodeList（支持 `forEach`），若无匹配项则返回空 NodeList。
:::

### 3.4.3. 在控制台使用 `$0` 选择元素

在浏览器开发者工具中，可以通过 `$0` 直接选择当前**高亮的 DOM 元素**（右键元素 → "审查元素" → 高亮的元素即为 `$0`）。
...
:::warning
**常见错误**：直接删除元素可能导致引用失效，建议先获取元素再操作。
:::
...
<script>
  ...
  // 解绑事件
  button.removeEventListener("click", handleClick); // 常在元素生命周期结束时执行
</script>
```
...
```html
<!-- 开发版（.js）包含注释，便于调试；生产版（.min.js）是压缩后的代码，体积小 -->
