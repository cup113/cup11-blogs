...
- **相同点**：区分大小写，不能以数字开头。
- **不同点**：
  - JavaScript 允许 `$` 和 `_` 作为开头或中间字符（如 `$price`, `_name`）。
  - JavaScript 变量名不能是保留字（如 `var`, `let`）。
...
⚠️ **注意**：`var` 的作用域是函数级而非块级，可能导致意外覆盖变量。
...
::: warning
与 Python 的不同：JavaScript 要求显式声明变量（如 `let x=5`），而 Python 直接写 `x=5` 即可。
:::
