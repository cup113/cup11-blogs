...
![效果图](./images/05-preview.png) #TODO 更改
...
2. 浏览器 API
   1. `setInterval(callback, interval)`：设置定时器。#TODO
...
1. `String.prototype.padStart(targetLength, padString)`：用于在字符串前面填充指定字符，达到指定长度。 <!--TODO explain it-->
...
1. `transition` 过渡功能。 #TODO
2. `setTimeout` 定时任务。 #TODO
