...
- `block` 块级元素，支持宽高设定，遵循块级布局（默认换行）。
- `inline-block` 行内块级元素，支持宽高设定，遵循行内布局（默认不换行）。
...
div {
  font-size: 2rem;
  font-family: "PingFang SC", "楷体", sans-serif; /* 尝试上网搜索常见字体名，进行修改。 */
}
```
...
1. 可以设置**多个字体**作为**回退方案**。如果没有第一个字体（或者不支持对应文本，如英文字体不支持中文）就用第二个，第二个没有就用第三个……比 Word 只能设置一个字体贴心多了。
...
该属性常与**固定宽高**配合使用。忘记设置容器尺寸时，`overflow` 可能不生效。
...
`border-radius` 一般只使用单值，表示四角**圆角的半径**。可以想到，如果元素为正方形，圆角半径为 `50%` 宽/高，那么整个元素就变成了圆形。
...
- `text-align` 作用于文本容器内部 `left` / `center` / `right`，居中元素本身需用 `margin: 0 auto;`
- `font-family` 需要设置回退字体和通用字体族。字体间用逗号隔开，字体有空格时外层要添加引号
...
:::details 参考答案
1. `B`
2. `margin: 0 auto` （备注：0 是可以改的，只是用作占位）
3. `text-align: right;`
